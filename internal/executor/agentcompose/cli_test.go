...
func TestParseRuntimeEnvelope(t *testing.T) {
	...
	encoded := `{"status":"completed","exit_code":0,"error":"","output":{"protocol":"transcript"},"result_json":"{\"finalText\":\"{}\"}"}`
	...
	object := `{"status":"completed","exit_code":0,"result_json":{"finalText":"{}"}}`
...
func TestRuntimeProviderOutputScenarios(t *testing.T) {
	validEnvelope := func(status, resultJSON string) string {
		return `{"status":` + strconv.Quote(status) + `,"exit_code":0,"error":"","output":"transcript secret prompt","result_json":` + strconv.Quote(resultJSON) + `}`
	}
	resultJSON := func(finalText string) string { return `{"finalText":` + strconv.Quote(finalText) + `}` }
	cases := []struct {
		name string
		raw  string
		want domain.RunStatus
	}{
		{"non-json transcript", validEnvelope("completed", resultJSON(validEventOutput)), domain.RunStatusSucceeded},
		{"invalid finalText", validEnvelope("completed", resultJSON("not-json")), domain.RunStatusFailed},
		{"empty finalText", validEnvelope("completed", resultJSON("")), domain.RunStatusFailed},
		{"missing result_json", `{"status":"completed","exit_code":0,"error":"","output":"transcript secret prompt"}`, domain.RunStatusFailed},
		{"invalid result_json", validEnvelope("completed", `"not-json"`), domain.RunStatusFailed},
		{"runtime failed", validEnvelope("failed", resultJSON(validEventOutput)), domain.RunStatusFailed},
		{"markdown code block", validEnvelope("completed", resultJSON("```json\n"+validEventOutput+"\n```")), domain.RunStatusFailed},
		{"trailing finalText", validEnvelope("completed", resultJSON(validEventOutput+" trailing")), domain.RunStatusFailed},
	}
	...
	cases = append(cases, struct {
		...
		raw  string
		want domain.RunStatus
	}{"valid-looking transcript", validLookingTranscript, domain.RunStatusFailed})
...
func TestExecutorTruncationDoesNotPublishBusinessArtifacts(t *testing.T) {
	...
	runtime := `{"status":"completed","exit_code":0,"output":"transcript","result_json":` + strconv.Quote(`{"finalText":`+strconv.Quote(validEventOutput)+`}`) + `}`
...
func TestExecutePreservesControlledEnvelopeAndSeparateRawOutput(t *testing.T) {
	...
	raw := `{"status":"completed","exit_code":0,"error":"","output":"transcript ` + prompt + `","result_json":` + strconv.Quote(`{"finalText":`+strconv.Quote(finalText)+`}`) + `}`
	executor := testExecutor(t, raw, 0)
...
func TestExecuteUsesRequestTimeout(t *testing.T) {
	runtime := `{"status":"completed","exit_code":0,"result_json":` + strconv.Quote(`{"finalText":`+strconv.Quote(validEventOutput)+`}`) + `}`
	for _, test := range []struct {
		name    string
		want    time.Duration
		request time.Duration
	}{
		{name: "request timeout", want: 2 * time.Minute, request: 2 * time.Minute},
		{name: "configured fallback", want: time.Second},
	} {
		t.Run(test.name, func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "agent-compose")
			script := "#!/bin/sh\nprintf '%s' \"$3\" >&2\nprintf '%s' '" + runtime + "'\n"
			...
			for _, artifact := range result.Result.Artifacts {
				...
				if string(data) != test.want.String() {
